...
ln -s ${TESTP_REPO_PATH}/Game/Plugins ${GDK_HOME}
...
${ENGINE_DIRECTORY}/Engine/Build/BatchFiles/Mac/Build.sh -projectfiles -project="${TEST_REPO_UPORJECT_PATH}" -game -engine -progress
if [[ $? -ne 0 ]]; then
...
${ENGINE_DIRECTORY}/Engine/Build/BatchFiles/Mac/XcodeBuild.sh ${BUILD_TARGET} ${BUILD_PLATFORM} ${BUILD_STATE} "${TEST_REPO_UPORJECT_PATH}" -buildscw
if [[ $? -ne 0 ]]; then
