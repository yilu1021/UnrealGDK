...
generate_build_configuration_steps () {
    ...
    if [[ -z "${BUILD_ALL_CONFIGURATIONS+x}" ]]; then
        ...
        upload_build_configuration_step "${ENGINE_COMMIT_HASH}" "Mac" "UE4Editor" "Development" "./ci/setup-build-gdk.sh"
    else
        ...
        for BUILD_STATE in "DebugGame" "Development"; do
            ...
            upload_build_configuration_step "${ENGINE_COMMIT_HASH}" "Mac" "UE4Editor" "${BUILD_STATE}"  "./ci/setup-build-gdk.sh"
        done
